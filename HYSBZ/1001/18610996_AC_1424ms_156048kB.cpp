...
#include <cstdio>
#include <vector>
const int MAXN = 1000010; const int INF = 2147483647;
struct node {
    int v, f, index;
...
#define getchar() \
(S == T && (T = (S = BB) + fread(BB, 1, 1 << 15, stdin), S == T) ? EOF : * S ++)
...
inline int hlpp(int n, int s, int t) {
    ...
    for(int u; highestActive >=0;){
        if(list[highestActive].empty()) {--highestActive; continue;}
...
int main() {
    ...
    for (int i=1;i<=n;i++){
        for (int j=1;j<m;j++){
            read(w);addEdge((i-1)*m+j, (i-1)*m+j+1, w);
        }
    }
    for (int i=1;i<n;i++){
        for (int j=1;j<=m;j++){
            read(w);addEdge((i-1)*m+j,i*m+j,w);
        }
    }
    for (int i=1;i<n;i++){
        for (int j=1;j<m;j++){
            read(w);addEdge((i-1)*m+j,i*m+j+1,w);
        }
    }
    printf("%d\n", hlpp(n*m + 1, s, t));
    return 0;
