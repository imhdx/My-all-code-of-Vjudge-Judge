...
Wint& operator*=(Wint &a,const Wint &b)
{
    ...
    return a.assign(ax.begin(),ax.end()),a;
}
*/
int temp[100005];
int main()
{
    ...
    while (T--)
    {
        ...
        vector<ll> b(maxx+1,0);
        for (int i=1;i<=n;i++) a[temp[i]]+=1,b[temp[i]]+=1;
        vector<ll> ax(a.begin(),a.end()),bx(b.begin(),b.end());
        ax=FFT(a.size()+b.size()).ask(ax,bx);
        //for (int i=0;i<ax.size();i++) printf("%d %lld\n",i,ax[i]);
        for (int i=1;i<=n;i++) ax[temp[i]+temp[i]]--;
        for (int i=0;i<ax.size();i++) ax[i]>>=1;
        for (int i=1;i<ax.size();i++) ax[i]+=ax[i-1];
        sort(temp+1,temp+1+n);
        long long ans=0;
        for (int i=1;i<=n;i++)
        {
            ans+=ax.back()-ax[temp[i]];
            ans-=1ll*(i-1)*(n-i);
            ans-=1ll*(n-i)*(n-i-1)/2;
            ans-=n-1;
        }
